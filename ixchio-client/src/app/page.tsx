...
function ThemeToggle({ dark, toggle }: { dark: boolean; toggle: () => void }) {
  return (
    <button onClick={toggle} className="t-muted hover:t-fg transition-colors text-xs"
...
function Sources({ sources }: { sources: Source[] }) {
  ...
  return (
    <div style={{ borderTop: "1px solid var(--border)" }} className="mt-6 pt-3">
      <button onClick={() => setOpen(!open)} className="text-xs t-muted hover:t-fg transition-colors">
        ...
        <div className="mt-2 space-y-1">
          {sources.map((s, i) => (
            <a key={i} href={s.url} target="_blank" rel="noopener noreferrer"
              className="block text-xs t-muted hover:t-fg truncate transition-colors">
              {s.title || s.url}
            </a>
          ))}
        </div>
...
// CSS-variable-driven inline styles
const S = {
  border: { border: "1px solid var(--border)" },
  borderB: { borderBottom: "1px solid var(--border)" },
  bg: { background: "var(--bg)" },
  surface: { background: "var(--surface)" },
  accent: { background: "var(--accent)", color: "var(--accent-fg)" },
  grad: { background: "linear-gradient(to top, var(--bg), var(--bg), transparent)" },
  bar: { background: "var(--accent)" },
  barTrack: { background: "var(--border)" },
};
...
export default function Home() {
  ...
  if (messages.length === 0) return (
    <div className="h-screen flex flex-col items-center justify-center px-4">
      ...
      <div className="w-full max-w-xl text-center">
        <h1 className="text-4xl md:text-5xl font-semibold tracking-tight mb-3" style={{ color: "var(--fg)" }}>ixchio</h1>
        <p className="text-sm mb-12" style={{ color: "var(--muted)" }}>Research any topic. Get a comprehensive report.</p>

        <form onSubmit={e => { e.preventDefault(); go(query.trim()); }} className="w-full">
          <div className="flex items-center rounded-lg px-4 py-3 transition-colors" style={S.border}>
            <input
            ...
            <select value={depth} onChange={e => setDepth(e.target.value as typeof depth)}
              className="text-xs bg-transparent outline-none cursor-pointer mr-3"
            ...
            <button type="submit" disabled={!query.trim()}
              className="text-xs font-medium px-4 py-1.5 rounded-md disabled:opacity-20 hover:opacity-80 transition-opacity"
              style={S.accent}>
              Research
            </button>
          </div>
        ...
        <div className="mt-6 flex items-center justify-center gap-4 text-xs" style={{ color: "var(--muted)" }}>
          <button onClick={() => go("demo")} className="hover:opacity-70 transition-opacity">Try demo</button>
          {history.length > 0 && <>
            <span style={{ color: "var(--border)" }}>·</span>
            <button onClick={() => setShowHist(!showHist)} className="hover:opacity-70 transition-opacity">
        ...
        {showHist && history.length > 0 && (
          <div className="mt-4 rounded-lg max-h-48 overflow-y-auto text-left" style={S.border}>
            {history.map(h => (
              <button key={h.task_id} onClick={() => loadHist(h)}
                className="w-full px-4 py-2.5 text-left transition-colors"
                style={{ borderBottom: "1px solid var(--border2)" }}>
                <p className="text-xs truncate" style={{ color: "var(--fg)" }}>{h.query}</p>
  ...
  return (
    <div className="flex flex-col h-screen">
      <header className="h-11 flex items-center justify-between px-5 shrink-0" style={S.borderB}>
        <span className="text-sm font-semibold tracking-tight" style={{ color: "var(--fg)" }}>ixchio</span>
        <div className="flex items-center gap-3 text-xs" style={{ color: "var(--muted)" }}>
          {searching && <span className="animate-pulse">Researching...</span>}
          {history.length > 0 && (
            <button onClick={() => setShowHist(!showHist)} className="hover:opacity-70 transition-opacity">History</button>
          )}
          <ThemeToggle dark={dark} toggle={toggle} />
        </div>
      </header>
      ...
      {showHist && (
        <div className="px-5 py-2 max-h-40 overflow-y-auto" style={S.borderB}>
          {history.map(h => (
            <button key={h.task_id} onClick={() => loadHist(h)}
              className="block w-full text-left py-1.5 text-xs truncate transition-colors"
              style={{ color: "var(--muted)" }}>
              {h.query}
            </button>
          ))}
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-0 pb-32">
        <div className="max-w-2xl mx-auto py-8 space-y-6">
          {messages.map((msg, i) => (
            <div key={i}>
              {msg.role === "user" ? (
                <p className="text-sm font-medium mb-1" style={{ color: "var(--fg)" }}>{msg.content}</p>
              ) : msg.status === "loading" || (msg.status !== "completed" && msg.status !== "error") ? (
                <div className="py-4">
                  <p className="text-sm" style={{ color: "var(--muted)" }}>{msg.currentStep || "Processing..."}</p>
                  <div className="mt-2 h-px overflow-hidden" style={S.barTrack}>
                    <div className="h-full transition-all duration-500" style={{ ...S.bar, width: `${msg.progress || 0}%` }} />
                  </div>
                  <p className="text-[11px] mt-1 tabular-nums" style={{ color: "var(--muted)" }}>{msg.progress || 0}%</p>
                </div>
              ) : msg.status === "error" ? (
                <div className="py-3">
                  ...
                  {msg.query && (
                    <button onClick={() => go(msg.query!)}
                      className="mt-1.5 text-xs underline transition-colors hover:opacity-70"
                      style={{ color: "var(--muted)" }}>
                      Retry
                    </button>
                  )}
                ...
                <div>
                  <div className="flex items-center gap-3 mb-4 text-xs" style={{ color: "var(--muted)" }}>
                    <span>Report</span>
                    <button onClick={() => doCopy(msg.content, i)} className="hover:opacity-70 transition-opacity">
                      {copied === i ? "Copied" : "Copy"}
                    </button>
                    <button onClick={() => dlMd(msg.content, messages[i - 1]?.content || "research")}
                      className="hover:opacity-70 transition-opacity">Download</button>
                  </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 pt-8 pb-4 px-4" style={S.grad}>
        <form onSubmit={e => { e.preventDefault(); go(query.trim()); }} className="max-w-2xl mx-auto">
          <div className="flex items-center rounded-lg px-4 py-3 transition-colors" style={{ ...S.border, background: "var(--bg)" }}>
            <input
              type="text" value={query} onChange={e => setQuery(e.target.value)}
              placeholder="Ask a follow-up or new topic..."
              disabled={searching} maxLength={500} spellCheck={false}
              className="flex-1 bg-transparent text-sm outline-none disabled:opacity-30"
              style={{ color: "var(--fg)" }}
            />
            <select value={depth} onChange={e => setDepth(e.target.value as typeof depth)}
              disabled={searching}
              className="text-xs bg-transparent outline-none cursor-pointer mr-3 disabled:opacity-30"
              style={{ color: "var(--muted)" }}>
              <option value="shallow">Quick</option>
              <option value="medium">Standard</option>
              <option value="deep">Deep</option>
